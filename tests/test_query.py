...
import src.api.handlers.query as handler
...
def _patch_basics(monkeypatch, service_factory):
    ...
    monkeypatch.setattr(handler, "get_query_settings", lambda: settings)
    monkeypatch.setattr(handler, "get_clients", lambda region: object())
    monkeypatch.setattr(handler, "QueryService", service_factory)


def test_query_handler_options(monkeypatch):
    def factory(*_a, **_k):
...
def test_query_handler_bad_json(monkeypatch):
    class DummyService:
...
def test_query_handler_success(monkeypatch):
    class DummyService:
        ...
        def execute(self, payload):
            return {"columns": ["c1"], "rows": [["v1"]], "payload": payload}

    _patch_basics(monkeypatch, DummyService)

    response = handler.lambda_handler(_event(body={"sql": "SELECT 1"}), None)
    assert response["statusCode"] == 200
    body = json.loads(response["body"])
    assert body["columns"] == ["c1"]
    assert response["headers"]["Access-Control-Allow-Origin"] == "http://localhost:5173"


def test_query_handler_domain_error(monkeypatch):
...
def test_query_handler_unexpected(monkeypatch):
    class FailingService:
        def __init__(self, *_a, **_k):
            pass
